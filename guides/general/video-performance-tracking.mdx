...
description: "Complete guide to video performance tracking across TikTok, YouTube, Instagram, and Twitter. Get hourly metrics, bot detection, and growth analytics."
---
...
<CardGroup cols={2}>
  ...
  <Card title="Detect Fraud" icon="shield-check">
    Identify fake engagement with AI-powered bot detection
  </Card>
</CardGroup>
...
const tiktokMetrics = {
  ...
  botScore: "Fake engagement confidence (0-1)"
};
```
...
const youtubeMetrics = {
  ...
  likes: "Like count",
  comments: "Comment count",
  botScore: "Bot activity detection"
};
```
...
const instagramMetrics = {
  ...
  botScore: "Engagement authenticity"
};
```
...
if (performance.bot_confidence > 0.5) {
  console.warn('⚠️ High bot activity detected:', performance.bot_confidence);
}
```
...
```javascript
function calculateEngagementQuality(snapshot, botConfidence) {
  ...
  const authenticEngagement = engagementRate * (1 - botConfidence);
  
  return {
    rawEngagementRate: engagementRate.toFixed(2) + '%',
    authenticEngagementRate: authenticEngagement.toFixed(2) + '%',
    botPenalty: (botConfidence * 100).toFixed(1) + '%'
  };
}
```
...
<AccordionGroup>
  ...
  <Accordion icon="chart-line" title="Compare Benchmarks">
    ...
    const benchmark = {
      ...
      acceptableBotScore: 0.3
    };
    
    function compareTooBenchmark(video, benchmark) {
      ...
      return {
        ...
        botScore: video.bot_confidence < benchmark.acceptableBotScore ? '✅' : '⚠️'
      };
    }
    ```
  </Accordion>
