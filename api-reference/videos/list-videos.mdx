...
# Get all videos with high bot scores
...
# Filter by bot confidence
suspicious = [
    v for v in data['videos']
    if v['bot_confidence'] > 0.5
]

print(f"Found {len(suspicious)} videos with high bot activity")
```
  ...
  "videos": [
    ...
    },
    {
      ...
      "bot_analysis": {
        ...
        "engagement_velocity_score": 0.03,
        "comment_quality_score": 0.07
      },
...
async function getActiveVideos() {
  ...
  return data.videos.map(v => ({
    ...
    botScore: v.bot_confidence
  }));
}
```

### Find Videos by Bot Score

```python
def get_suspicious_videos(min_confidence=0.5):
    response = requests.get(
        'https://contentstats.io/api/v1/videos',
        headers={'X-API-Key': os.environ['CONTENTSTATS_API_KEY']}
    )
    ...
    return [
        v for v in data['videos']
        if v['bot_confidence'] >= min_confidence
    ]

# Get videos likely using bots
suspicious = get_suspicious_videos(0.5)
for video in suspicious:
    print(f"{video['video_link']}: {video['bot_confidence']:.2f}")
```
...
async function getPlatformStats(platform) {
  ...
  const stats = {
    ...
    avgBotScore: data.videos.reduce((sum, v) => sum + v.bot_confidence, 0) / data.total,
