...
<AccordionGroup>
  ...
  <Accordion icon="robot" title="High Bot Activity Detected">
    Video shows clear bot activity:
    
    ```javascript
    async function stopIfBotDetected(videoId, threshold = 0.7) {
      const video = await getVideo(videoId);
      
      if (video.bot_confidence > threshold) {
        await stopVideo(videoId);
        console.log('Stopped: High bot confidence', video.bot_confidence);
        return { botted: true, score: video.bot_confidence };
      }
    }
    ```
  </Accordion>
...
// Stop all completed or high-bot videos
const videos = await listVideos();
const toStop = videos.videos
  .filter(v => v.status === 'monitoring')
  .filter(v => v.bot_confidence > 0.7 || v.days_remaining < 0.5)
  .map(v => v.id);
...
console.log('Bot analysis:', stoppedVideo.bot_analysis); // Still available
```
...
<AccordionGroup>
  ...
  <Accordion icon="database" title="Log Stop Reasons">
    ...
    await stopWithReason('cm5abc123', 'High bot activity detected');
    ```
  </Accordion>
